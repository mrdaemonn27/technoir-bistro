        ...
        <main class="mx-auto max-w-5xl px-6 py-10 lg:px-8">
            <section class="admin-reveal rounded-[28px] border border-[#f0dfca] bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                    <div>
                        <p class="text-xs font-black uppercase tracking-[0.2em] text-[#b76b0c]">Edit Menu</p>
                        <h1 class="mt-2 text-3xl font-black tracking-tight md:text-4xl">{{ $menu->name }}</h1>
                        <p class="mt-3 max-w-2xl text-sm leading-7 text-[#6b5b4c]">
                            Perbarui detail menu, kategori, harga, foto, dan status ketersediaan.
                        </p>
                    </div>
            ...
            <section class="admin-reveal mt-6 rounded-[28px] border border-[#f0dfca] bg-white p-6 shadow-sm">
                @if ($errors->any())
                    <div class="mb-6 rounded-3xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-700">
                        <p class="font-black">Menu belum bisa diperbarui:</p>
                ...
                <form action="{{ route('admin.menus.update', $menu) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <input id="name" class="clean-input mt-2 px-4 py-3" type="text" name="name" value="{{ old('name', $menu->name) }}" required />
                    ...
                    <div>
                        ...
                        <input id="price" class="clean-input mt-2 px-4 py-3" type="number" name="price" value="{{ old('price', $menu->price) }}" min="0" step="1" required />
                        <x-input-error :messages="$errors->get('price')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="description" :value="__('Deskripsi')" class="text-[#1f1712]" />
                        <textarea id="description" name="description" class="clean-input mt-2 px-4 py-3" rows="4">{{ old('description', $menu->description) }}</textarea>
                        <x-input-error :messages="$errors->get('description')" class="mt-2" />
                    </div>

                    <div class="rounded-3xl border border-dashed border-[#e8d5bd] bg-[#fffdf9] p-5">
                        <x-input-label for="image" :value="__('Ganti Foto (Opsional)')" class="text-[#1f1712]" />
                        @if($menu->image)
                            <div class="mt-3 flex items-center gap-4 rounded-2xl bg-white p-3">
                                <img src="{{ filter_var($menu->image, FILTER_VALIDATE_URL) ? $menu->image : asset('storage/' . $menu->image) }}" class="h-20 w-20 rounded-2xl object-cover shadow-sm" alt="{{ $menu->name }}">
                                <p class="text-sm text-[#6b5b4c]">Foto menu saat ini.</p>
                            </div>
                        @endif
                        <input id="image" type="file" name="image" class="mt-4 block w-full text-sm text-[#6b5b4c] file:mr-4 file:rounded-full file:border-0 file:bg-[#fff3df] file:px-4 file:py-2 file:text-sm file:font-black file:text-[#b76b0c] hover:file:bg-[#ffe5bd]">
                        <x-input-error :messages="$errors->get('image')" class="mt-2" />
                    </div>

                    <label class="inline-flex items-center gap-3 rounded-full border border-[#f0dfca] bg-[#fffdf9] px-4 py-3">
                        <input type="checkbox" name="availability" value="1" {{ $menu->availability ? 'checked' : '' }} class="rounded border-[#e8d5bd] text-[#ea8e16] shadow-sm focus:ring-[#ea8e16]">
