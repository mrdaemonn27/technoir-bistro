    ...
    try {
      ...
      if (response.statusCode == 200 && data['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(data['message'] ?? 'Gagal memuat favorit'), backgroundColor: Colors.red),
        );
      }
    } catch (e) {
      setState(() => _isLoading = false);
  ...
  Future<void> _removeFavorite(int menuId, int index) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && data['success'] == true) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Dihapus dari daftar favorit'), duration: Duration(seconds: 1)),
        );
      }
    } catch (e) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('Belum ada menu tersimpan', style: TextStyle(color: Colors.grey[500], fontSize: 16)),
        ],
      ),
    );
  }
          ...
          ClipRRect(
            ...
            child: menu['image'] != null
                ? Image.network(
                    ...
                    height: 70,
                    fit: BoxFit.cover,
                    errorBuilder: (c, e, s) => Container(width: 70, height: 70, color: Colors.grey[200], child: const Icon(Icons.fastfood, color: Colors.grey)),
                  )
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Rp ${menu['price'] ?? 0}',
                  style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2C74B3)),
                ),
              ],
            ),
          ),
